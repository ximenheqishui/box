  ...
  methods: {
    ...
    getData: function getData(type) {
      ...
      this.api.getUser(postdata).then(function (res) {
        ...
        _this.searchLoading = false; // throw new Error('运算错误')
    ...
    getDepartment: function getDepartment() {
      ...
      this.api.getDepartment({}).then(function (res) {
        if (res.code === 0) {
          ...
          _this6.errorHandler(res.message || '获取部门失败');
        }
      }).catch(function (error) {
        _this6.errorHandler(error.message);
      });
    },
    // 获取所有的角色
    getRole: function getRole() {
      ...
      _this.api.getRole({}).then(function (res) {
        if (res.code === 0) {
          ...
          _this7.errorHandler(res.message || '获取角色失败');
        }
      }).catch(function (error) {
    ...
    getOption: function getOption() {
      ...
      this.api.getDictionaries({}).then(function (res) {
        if (res.code === 0) {
          _this8.sex = res.data.sex;
          _this8.status = res.data.status;
        } else {
          _this8.errorHandler(res.message || '获取数据字典失败');
        }
      }).catch(function (error) {
    ...
    showDialog: function showDialog(obj) {
      ...
      _this.$nextTick(function () {
        ...
        if (obj) {
          ...
          var data = JSON.parse(JSON.stringify(obj.row));
          data.password2 = '';
          data.password = '';
    ...
    deleteRow: function deleteRow(scope, rows) {
      ...
      _this.api.delUser({
        ...
        if (res.code === 0) {
          ...
          _this9.errorHandler(res.message || '删除失败');
        }
      }).catch(function (error) {
        _this9.errorHandler(error.message);
      });
    },
    // 批量删除
    deleteMore: function deleteMore() {
      ...
      this.api.delUser({
        ...
        if (res.code === 0) {
          ...
          _this10.errorHandler(res.message || '删除失败');
        }
      }).catch(function (error) {
        _this10.errorHandler(error.message);
      });
    },
    // 提交表单
