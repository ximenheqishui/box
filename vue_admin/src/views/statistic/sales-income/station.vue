    ...
    <div class="search">
      <el-form ref="form" :model="searchData" class="" label-width="30%" size="small">
        <el-row :gutter="0">
          ...
          <el-col :sm="8" :md="8" :lg="7" :xl="6" v-show="false">
          </el-col>
        </el-row>
        <el-row :gutter="0">
          <el-col :sm="8" :md="8" :lg="7" :xl="6">
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            {{ scope.row.startDate | filtersDate }}
          </template>
        </el-table-column>
        <el-table-column
          label="结束时间">
          <template slot-scope="scope">
            {{ scope.row.endDate | filtersDate }}
          </template>
        </el-table-column>
        <el-table-column
          prop="areaName"
...
<script>
  ...
  export default {
    ...
    data () {
      let now = new Date()
      let start = this.dateFmt('yyyy-MM', now)
      start += '-01' + ' 00:00:00'
      let end = this.dateFmt('yyyy-MM-dd', new Date())
      end += ' 23:59:59'
      return {
    ...
    computed: {
      downloadUrl: function () {
        return this.api.exportUrl.stationT + '/?' + qs.stringify(this.lastPostData, { arrayFormat: 'repeat' })
      }
    },
    watch: {
    },
    ...
    methods: {
      getSummaries (param) {
        ...
        columns.forEach((column, index) => {
          switch (index) {
          case columns.length - 4:
            sums[index] = '合计'
            break
          case columns.length - 3:
            sums[index] = _this.resultData.totalQuantity
            break
          case columns.length - 2:
            sums[index] = _this.resultData.totalAmount
            break
      ...
      getData (type) {
        ...
        if (type) {
          ...
          postdata.startDate = postdata.startDate ? this.dateFmt('yyyy-MM-dd hh:mm:ss', new Date(postdata.startDate)) : ''
          postdata.endDate = postdata.endDate ? this.dateFmt('yyyy-MM-dd hh:mm:ss', new Date(postdata.endDate)) : ''
        ...
        this.api.getStationT(postdata).then(res => {
          _this.pageLoading = false
          _this.searchLoading = false
          if (res.data.code === 0) {
            _this.resultData = res.data.data.detail2
            _this.resultData.totalAmount = res.data.data.totalAmount
            _this.resultData.totalQuantity = res.data.data.totalQuantity
          } else {
            _this.$message({
              type: 'error',
              showClose: true,
